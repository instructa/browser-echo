...
function browserEcho(opts = {}) {
  ...
  return {
    ...
    transformIndexHtml(html) {
      if (!options.enabled || !options.injectHtml)
        return;
...
function attachMiddleware(server, options) {
  const sessionStamp = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
  const logFilePath = join(options.fileLog.dir, `dev-${sessionStamp}.log`);
    ...
    collectBody(req).then((raw) => {
      ...
      for (const entry of payload.entries) {
        ...
        if (entry.stack && options.stackMode !== "none") {
          const lines = options.stackMode === "full" ? indent(entry.stack, "    ") : `    ${(String(entry.stack).split(/\r?\n/g).find((l) => l.trim().length > 0) || "").trim()}`;
        ...
        if (options.fileLog.enabled) {
          ...
          if (entry.stack && options.stackMode !== "none")
            toFile.push(indent(entry.stack, "    "));
          try {
            appendFileSync(logFilePath, toFile.join("\n") + "\n");
          } catch {
          }
        }
      }
